...
class TravelAgentClient:
    ...
    def query(self, question: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                "✗ Request timed out - agent may be processing or unresponsive"
            )
            return None
        except requests.exceptions.ConnectionError:
            logger.error(
                f"✗ Connection failed - is the agent running at {self.base_url}?"
            )
            return None
        except Exception as e:
    ...
    def get_conversation_info(self) -> Optional[dict]:
        ...
        try:
            ...
            data = response.json()
            logger.info(
                f"✓ Conversation info: {data['user_turns']} user turns, {data['assistant_turns']} assistant turns"
            )
            return data
        except Exception as e:
        ...
        try:
            ...
            if user_input.lower() == "info":
                ...
                if info:
                    ...
                    print(
                        f"   Active: {'Yes' if info['conversation_active'] else 'No'}\n"
                    )
                else:
